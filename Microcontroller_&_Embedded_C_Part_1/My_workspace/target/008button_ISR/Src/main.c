...
uint8_t volatile g_button_pressed = 0; //variable to check button pressed or not?

uint32_t g_button_press_count =0;//variable to store the button pressed count
	...
	while(1)
	{
		...
		*pEXTIMaskReg &= ~( 1 << 0);
		...
		if(g_button_pressed){
			...
			//Note: in higher optimization level, if i is not declared as volatile, compiler thinks i is an unused variable
			//and the instruction is not generated for this for loop. Hence i is declared as volatile to avoid this bug.
...
//This function gets executed every time the button is pressed
